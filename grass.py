...
async def main():
    # Get user ID and number of proxies from .env
    _user_id = os.getenv('USER_ID')
    if not _user_id:
        logger.error("USER_ID not found in .env file. Exiting.")
        return  # Exit if no user ID is provided

    try:
        num_proxies_to_use = int(os.getenv('NUM_PROXIES', 0))
        if num_proxies_to_use <= 0:
            raise ValueError("The number of proxies must be a positive integer.")
    except ValueError as e:
        logger.error(f"Invalid NUM_PROXIES in .env file: {e}. Exiting.")
